...
def main():
    ...
    for line in tqdm(lines):
        ...
        # 1. PLL (Log-Likelihood) 계산
        ll_score = calculator.get_sentence_score(text)

        if label == "ok":
            ok_scores.append(ll_score)
        elif label == "violation":
            viol_scores.append(ll_score)

        # 2. Calibration 계산
        pred_label, confidence = calculator.get_calibration_prob(text)
        is_correct = (pred_label == label)
        calib_data.append((is_correct, confidence))
    ...
    print("\n" + "=" * 40)
    print(f" [Metric 1] PLL Gap Analysis ({CONDITION})")
    print("=" * 40)
    ...
    print("\n" + "=" * 40)
    print(f" [Metric 2] Calibration Analysis")
    print("=" * 40)
